...
class TempSave {
    ...
    // dummyExpenseData를 위한 프로퍼티
    let dummyExpenseName = ["DummyExpense0"]
    let dummyExpensePayDate = Calendar.current.date(byAdding: .day, value: -1, to: nowDate)!
    let dummyExpenseCurrency = 0
    let dummyExpenseExchangeRate = 0
    let dummyExpenseInfo = ["DummyExpenseInfo0"]
    let dummyExpenseCountry = 0
    let dummyExpenseLocation = ["DummyExpenseLocation0"]
    let dummyExpenseParticipantArray = [[String]]()
    let dummyExpensePaymentMethod = 0
    let dummyExpensePayAmount = 0.0
    let dummyExpenseCategory = 0
    ...
    func addExpense(travel: Travel) {
        ...
        tempExpense.currency = Int64(dummyExpenseCurrency)
        tempExpense.exchangeRate = Double(dummyExpenseExchangeRate)
        tempExpense.info = dummyExpenseInfo.first
        tempExpense.country = Int64(dummyExpenseCountry)
        tempExpense.location = dummyExpenseLocation.first
        tempExpense.participantArray = dummyExpenseParticipantArray.first
        tempExpense.paymentMethod = Int64(dummyExpensePaymentMethod)
        tempExpense.payAmount = Double(dummyExpensePayAmount)
        tempExpense.payDate = dummyExpensePayDate
        tempExpense.category = Int64(dummyExpenseCategory)
