                    ...
                    geocoder.reverseGeocodeLocation(currentLocation) { (placemarks, _) in
                        if let currentPlacemark = placemarks?.first {
                            ...
                            if let baseTimeZone = basePlacemark.timeZone, let currentTimeZone = currentPlacemark.timeZone {
                                let timeDifference = currentTimeZone.secondsFromGMT(for: nowDate) - baseTimeZone.secondsFromGMT(for: nowDate)
                                // base가 3시일 때 current가 5시이면 timeDifference는 7200. 저장하기 전에 date에 7200초를 더하면 base 기준 5시인 date가 된다.
                                tbr = date.addingTimeInterval(TimeInterval(timeDifference))
                            } else {
                                print("Failed to fetch time zones.")
                            }
                        } else {
                            print("Failed to fetch current placemark.")
                        }
                    }
                }
            } else {
                print("Failed to fetch base placemark.")
            }
        }
        return tbr
    }
